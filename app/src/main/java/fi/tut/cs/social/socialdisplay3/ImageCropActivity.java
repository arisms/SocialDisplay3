    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == RESULT_CROP ) {
            if(resultCode == Activity.RESULT_OK){
                ...
                File file = new File(getApplicationContext().getFilesDir(), "SocialDisplayPicturesBuffer");

                // Create the file if it doesn't already exist
                if (!file.exists()) {
                    Log.d("ImageCropActivity", "SocialDisplayPictures - file does not exist");
                    try {
                        file.createNewFile();
                    } catch (IOException e) {
                        e.printStackTrace();
                ...
                Intent imageSelectedIntent = new Intent(NewProfileFragment.IMAGE_SELECTED);
                FileOutputStream outputStream = null;
                try {
                    //outputStream = new FileOutputStream("N2Uavatar");
                    outputStream = getApplicationContext().openFileOutput("SocialDisplayPicturesBuffer", Context.MODE_PRIVATE);
